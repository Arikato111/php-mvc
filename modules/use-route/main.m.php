...
/*  * Copyright (c) 2022 ณวสันต์ วิศิษฏ์ศิงขร
    ...
    * Original code : https://github.com/Arikato111/wisit-express
*/
class Route
{
    ...
    public static function Routing(string $method, string $path, callable $callback): null | callable
    {
    ...
    {
        $value = Route::Routing('GET', $path, $callback);
        if ($value) {
            echo $value(Route::request(), Route::response());
            exit;
        }
    }
    ...
    {
        $value = Route::Routing('POST', $path, $callback);
        if ($value) {
            echo $value(Route::request(), Route::response());
            exit;
        }
    ...
    {
        $value = Route::Routing('PUT', $path, $callback);
        if ($value) {
            echo $value(Route::request(), Route::response());
            exit;
        }
    ...
    {
        $value = Route::Routing('DELETE', $path, $callback);
        if ($value) {
            echo $value(Route::request(), Route::response());
            exit;
        }
    ...
    {
        $value = Route::Routing("$_SERVER[REQUEST_METHOD]", $path, $callback);
        if ($value) {
            echo $value(Route::request(), Route::response());
            exit;
        }
